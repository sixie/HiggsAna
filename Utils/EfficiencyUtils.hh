#ifndef HIGGSANA_UTILS_EFFICIENCYUTILS_H
#define HIGGSANA_UTILS_EFFICIENCYUTILS_H
...
TGraphAsymmErrors* createEfficiencyGraph(TH1F* numerator, TH1F* denominator,
  ...
  TH1F *d = denominator;  
  if (bins.size() > 0) {   
    n = higgsana::rebin(numerator,bins);
    d = higgsana::rebin(denominator,bins);
  }
...
//--------------------------------------------------------------------------------------------------
  ...
  if (xbins.size() > 0 && ybins.size() > 0) {
    n = higgsana::rebin(numerator,xbins,ybins);
    d = higgsana::rebin(denominator,xbins,ybins);
  }
