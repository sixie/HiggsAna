...
void selectEleHZZICHEP2012WPWithTightTagPerFile(const string inputfile,          // input file
  ...
  if (!matchGen) {
    hasJSON = kTRUE;
    rlrm.AddJSONFile("/afs/cern.ch/work/s/sixie/public/HZZ4l/auxiliar/2012/Cert_HCP_53X_JSON.txt"); 
  }
  ...
  for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {
    ...
    for(Int_t i=0; i<electronArr->GetEntriesFast(); i++) {
      ...
      if (dataType == 1) {
        ...
        //if we're using single electron triggers, require tag pt > 30 GeV to avoid trigger turn-on
