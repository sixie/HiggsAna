...
void plotEff_Zto4l(const TString conf,          // input file
...
//   const Double_t ylow  = 0.6;
  ...
  if(sigModPass==2 || sigModFail==2) {
    generateHistTemplates(mcfilename,ptBinEdgesv,etaBinEdgesv,phiBinEdgesv,npvBinEdgesv,rhoBinEdgesv,massLo,massHi,doAbsEta,charge);
  }
  if(sigModPass==4 || sigModFail==4) {
    generateDataTemplates(mcfilename,ptBinEdgesv,etaBinEdgesv,phiBinEdgesv,npvBinEdgesv,rhoBinEdgesv,massLo,massHi,doAbsEta,charge);
  }
  ...
  TH1F *puWeights = 0;
//   TFile *pufile = new TFile("puweights_s3.root");  
		           ...
		           const Double_t massLo, const Double_t massHi, const Bool_t doAbsEta, const Int_t charge)
{
  ...
  for(UInt_t ientry=0; ientry<intree->GetEntries(); ientry++) {
    ...
    if(data.pass) {
      passPt[ipt]->Fill(data.mass, data.weight);
      passEta[ieta]->Fill(data.mass, data.weight);
      passPhi[iphi]->Fill(data.mass, data.weight);
      passEtaPt[ipt*etaNbins + ieta]->Fill(data.mass, data.weight);
      passEtaPhi[iphi*etaNbins + ieta]->Fill(data.mass, data.weight);
      passNPV[inpv]->Fill(data.mass, data.weight);
      passRho[irho]->Fill(data.mass, data.weight);
    } else {
      failPt[ipt]->Fill(data.mass, data.weight);
      failEta[ieta]->Fill(data.mass, data.weight);
      failPhi[iphi]->Fill(data.mass, data.weight);
      failEtaPt[ipt*etaNbins + ieta]->Fill(data.mass, data.weight);
      failEtaPhi[iphi*etaNbins + ieta]->Fill(data.mass, data.weight);
      failNPV[inpv]->Fill(data.mass, data.weight);
      failRho[irho]->Fill(data.mass, data.weight);
    }    
  }
  infile.Close();
 
  TFile outfile(Form("%s/histTemplates.root",CPlot::sOutDir.Data()), "RECREATE");
  cout << "Output Template file: " << Form("%s/histTemplates.root",CPlot::sOutDir.Data()) << endl;
  for(UInt_t ibin=0; ibin<ptNbins; ibin++) {
    passPt[ibin]->Write();
    failPt[ibin]->Write();
    delete passPt[ibin];
...
//   if (ibin < 3) return;
  ...
  if(sigpass==2 || sigfail==2) {
    histfile = new TFile(Form("%s/histTemplates.root",CPlot::sOutDir.Data()));
    assert(histfile);
  }
  TFile *datfile = 0;
...
//
  ...
  RooRealVar eff("eff","Efficiency",0.90,0,1.0);
  RooRealVar NbkgPass("NbkgPass","Background count in PASS sample",10,0,40000);
  if(bkgpass==0) NbkgPass.setVal(0);
  RooRealVar NbkgFail("NbkgFail","Background count in FAIL sample",0.01*NbkgFailMax,0.0,NbkgFailMax);  
...
RooExtendPdf esignalFail("esignalFail","esignalFail",*(sigFail->model),NsigFail,"signalRange");
RooExtendPdf ebackgroundFail("ebackgroundFail","ebackgroundFail",*(bkgFail->model),NbkgFail,"signalRange");
RooAddPdf modelFail("modelFail","Model for FAIL sample", (bkgpass>0) ? RooArgList(esignalFail,ebackgroundFail) : RooArgList(esignalFail));
