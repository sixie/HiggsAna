...
//           if (gen->status == 1 && (abs(gen->pdgid) == 11 || abs(gen->pdgid) == 13 ) ) {
          ...
          for(Int_t l=k+1; l<genparticleArr->GetEntries(); l++) {
            ...
            if (minGenLeptonDR > higgsana::deltaR(gen->eta,gen->phi,p2->eta,p2->phi)) {
              minGenLeptonDR = higgsana::deltaR(gen->eta,gen->phi,p2->eta,p2->phi);
            }
          }
        }
        ...
        for(uint i=0; i<uint(muonArr->GetEntries()); i++) 
        {
          ...
          if( printDebug ) { 
           ...
           if (!PassMuonHZZ4lPreselectionWithoutPtCut(mu)) {
//           if (!PassMuonHZZ4lPreselection(mu)) {
        ...
        for(uint i=0; i<uint(electronArr->GetEntries()); i++) 
        {
          ...
          if (overlapsWithMuon) {
           ...
           if (!PassEleHZZ4lPreselectionWithoutPtCut(ele)) {
//           if (!PassEleHZZ4lPreselection(ele)) {
        ...
        //********************************************************
        for( uint i=0; i<leptonType.size(); i++ ) {
          selectedLeptonPt.push_back(leptonFourVector[i].Pt());
          selectedLeptonEta.push_back(leptonFourVector[i].Eta());
          selectedLeptonPhi.push_back(leptonFourVector[i].Phi());
          selectedLeptonCharge.push_back(1);
        }
          ...
          for(uint j = i+1; j < leptonType.size(); ++j) {
            ...
            if( doFSR ) { 
              ...
              if( printDebug ) { 
                ...
                cout << "isolated_i: " << passIsolation_i << " - isolated_j: " << passIsolation_j << endl;
              }
	  
            } // doFSR
            else {
              passIsolation_i = kTRUE;
              passIsolation_j = kTRUE;             
            }
...
//         }
        ...
        for( uint l=0; l<genpt.size(); ++l ) {
          ...
          for( uint k=0; k<selectedLeptonPt.size(); ++k ) {
            Double_t tmpDR = higgsana::deltaR(geneta[l],genphi[l],selectedLeptonEta[k],selectedLeptonPhi[k]);
