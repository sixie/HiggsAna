...
void HwwNtuplerMod::SlaveBegin()
{
  ...
  fGenParticleArr = new TClonesArray("mithep::TGenParticle");     assert(fGenParticleArr);
  fPFCandidateArr = new TClonesArray("mithep::TPFCandidate",2000);     assert(fPFCandidateArr);
...
//           }
  ...
  for (UInt_t i=0; i<GenPhotons->GetEntries(); ++i) {
    ...
    pGenParticle->motherPdgID  = 0;
    if (GenPhotons->At(i)->Mother()) {
      pGenParticle->motherPdgID  = GenPhotons->At(i)->Mother()->PdgId();
    }
  }
