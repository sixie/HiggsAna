...
void selectEleHWWICHEP2012WPPerFile(const string inputfile,          // input file
  ...
  for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {
    ...
    for(Int_t i=0; i<electronArr->GetEntriesFast(); i++) {
      ...
      if (!(passElectronTriggerDenominator(tag) && PassEleHWWICHEP2012IDMVA( tag, eleIDMVA) 
            && (ComputeElePFIso04(tag, pfcandidateArr, rhoEleIso,EleEAEra) / tag->pt < 0.15)
            )
        ) continue;
...
// 	    continue;
        ...
        //PASS
        //******************
        Bool_t passID = passElectronTriggerDenominator(probe) && PassEleHWWICHEP2012IDMVA( probe, eleIDMVA);
        Bool_t passIso = (ComputeElePFIso04(probe, pfcandidateArr, rhoEleIso,EleEAEra) / probe->pt < 0.15);
        Bool_t pass = passID && passIso;
