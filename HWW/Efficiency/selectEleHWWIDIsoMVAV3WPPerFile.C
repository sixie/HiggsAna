...
void selectEleHWWIDIsoMVAV3WPPerFile(const string inputfile,          // input file
  ...
  for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {
    ...
    for(Int_t i=0; i<electronArr->GetEntriesFast(); i++) {
      ...
      if (!(passElectronTriggerDenominator(tag) && PassEleHWWIDIsoMVAV3( tag, pfcandidateArr, rhoEleIso,EleEAEra, eleIDMVA))) continue;
...
// 	    continue;
        ...
        //PASS
        //******************
        Bool_t pass = passElectronTriggerDenominator(probe) && PassEleHWWIDIsoMVAV3( probe, pfcandidateArr, rhoEleIso,EleEAEra, eleIDMVA);
