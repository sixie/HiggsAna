...
void HZGammaSelection_mumug_Synchronization(Int_t Option = 0) 
{  
...
//   delete fPUFile;
  ...
  ofstream muList("MuonDump_sixie.txt");
...
//             ||
        ...
        for(uint i=0; i<uint(muonArr->GetEntries()); i++) 
        {
          ...
          if (PassSelectionStages[0] && PassSelectionStages[1] && PassSelectionStages[2]) {
            muList << info->evtNum << " " << mu->pt << " " << mu->eta << " " << PassMuonHZGammaID(mu, (DataEra == kDataEra_2011_MC) ) << " " << PassMuonHZGammaIso(mu, pfcandidateArr, rhoMuonIso,MuonEAEra) << "\n";
          }
